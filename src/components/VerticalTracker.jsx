...
export default function VerticalTracker({ index = -1, total = 0, onSelect = () => {}, showArrow = true }) {
  ...
  return (
    <div className="group flex items-center gap-3 bg-transparent px-2 py-2 rounded-md pointer-events-auto">
      {showArrow && (
        <div className="flex items-center justify-center w-6 h-6 text-white opacity-70">
      ...
      <div className="flex flex-col items-center gap-2 overflow-hidden">
        <div className={`text-sm select-none text-white opacity-50 group-hover:opacity-100 ${blink ? 'tracker-blink' : ''}`}>
        ...
        <div className="flex flex-col items-center gap-2 overflow-hidden">
          {Array.from({ length: total }).map((_, i) => (
            <button
              ...
              className={`rounded-full h-1 transition-all duration-200 ease-out focus:outline-none focus:ring-2 focus:ring-white cursor-pointer ${
                  i === index
                    ? 'w-10 sm:w-12 bg-white opacity-100'
                    : 'w-8 sm:w-10 bg-white opacity-30 hover:w-10 sm:hover:w-12 hover:opacity-100 hover:bg-white'
                }`}
            />
          ))}
        </div>
